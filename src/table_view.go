...
func NewModel(patients []Patient) model {
...
func (p *Patient) ToRow() table.Row {
	return table.Row{
		p.ID,
		p.Name,
		fmt.Sprintf("%d", p.Age),
...
func GetMaxWidth(patients []Patient, field string) int {
	maxWidth := 0
...
func NewPatientTable(patients []Patient) table.Model {
	columns := []table.Column{
		{Title: "ID", Width: 8},
		{Title: "Name", Width: GetMaxWidth(patients, "Name")},
		{Title: "Age", Width: 3},
		...
		{Title: "Appointment Date", Width: GetMaxWidth(patients, "Appointment Date")},
	}
